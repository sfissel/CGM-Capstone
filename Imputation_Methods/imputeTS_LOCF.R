...
test_missing_lists$locf_predictions <- lapply(test_missing_lists$Value_Missing, function(x) na_locf(x))

locf_interp <- test_missing_lists|>
  rowwise() |>
  mutate(na_indices = list(which(is.na(Value_Missing)))) |>
  ungroup()

eval_data <- inner_join(locf_interp, test_complete_lists, by =c('SID','Day'))

locf_data <- eval_data |>
  unnest(locf_predictions)|>
  group_by(SID,Day) |>
  mutate(DailyTI_Index = row_number()-1) |>
  ungroup()|>
  select(SID,Day,DailyTI_Index,locf_predictions)

write_csv(locf_data, './Documents/UVA_MSDS/Capstones/Glucose/CGM_data/Locf_Test_Prediction.csv')
