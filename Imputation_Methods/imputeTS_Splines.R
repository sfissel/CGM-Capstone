...
test_missing_lists$spline_predictions <- lapply(test_missing_lists$Value_Missing, function(x) na_interpolation(x, option = 'spline'))

spline_interp <- test_missing_lists|>
  rowwise() |>
  mutate(na_indices = list(which(is.na(Value_Missing)))) |>
  ungroup()

eval_data <- inner_join(spline_interp, test_complete_lists, by =c('SID','Day'))

spline_data <- eval_data |>
  unnest(spline_predictions)|>
  group_by(SID,Day) |>
  mutate(DailyTI_Index = row_number()-1) |>
  ungroup()|>
  select(SID,Day,DailyTI_Index,spline_predictions)

write_csv(spline_data, './Documents/UVA_MSDS/Capstones/Glucose/CGM_data/Spline_Test_Prediction.csv')
